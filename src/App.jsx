...
const PDFToPPTX = () => {
  ...
  useEffect(() => {
    ...
    // 1. 自動修正 Vite 預設的 CSS (解決黑色背景與排版問題)
    const fixStyles = () => {
      const style = document.createElement('style');
      style.innerHTML = `
        /* 強制覆蓋 Vite 預設樣式 */
        :root { font-family: Inter, system-ui, Avenir, Helvetica, Arial, sans-serif; line-height: 1.5; font-weight: 400; }
        body { margin: 0; display: block; place-items: unset; min-width: unset; min-height: unset; background-color: #f8fafc; color: #0f172a; }
        #root { max-width: none; margin: 0; padding: 0; text-align: left; width: 100%; }
        /* 隱藏 Canvas */
        canvas { display: none; }
      `;
    ...
    // 2. 載入外部 Script (Tailwind + PDF.js + PPTXGenJS)
    ...
    const initLibs = async () => {
      try {
        ...
        if (!window.pdfjsLib) {
          await loadScript(`https://cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjsVersion}/pdf.min.js`);
        }
        if (!window.PptxGenJS) {
        ...
        console.log("All libraries and styles loaded successfully");
        libsLoaded.current = true;
      } catch (err) {
        console.error("Failed to load libraries", err);
        setStatus('error');
        setStatusMessage('無法載入必要元件，請檢查網路連線。');
      }
    };
  ...
  const convertToPPTX = async () => {
    ...
    // 檢查核心是否載入
    if (!window.pdfjsLib || !window.PptxGenJS) {
      // 再次嘗試補救 Worker
      ...
      if (!window.pdfjsLib || !window.PptxGenJS) {
        setStatus('error');
        setStatusMessage('轉換核心尚未載入，請重新整理頁面再試。');
        return;
      }
    }
